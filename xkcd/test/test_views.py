...
class ViewTestCase(TestCase):
    ...
    def setUp(self):
        self.generic_user = Person.objects.create_user(
            first_name='miguel',
            last_name='barbosa',
            username='test-user',
            email='test@test.com',
...
# Testing post login section of site
    ...
    def test_all_user_likes(self):
        Like.objects.create(comic_name='test-name', url='http://test.com', liked_by=self.generic_user)
    ...
    def test_random_search_page(self, mock_requests):
        ...
        self.assertIn(
            '<h3>{} - {}</h3>'.format(mock_comic['safe_title'], mock_comic['year']),
            response.content
        )
        self.assertIn(
            '<img alt="{}" src="{}">'.format(mock_comic['alt'], mock_comic['img']),
            response.content
        )

        response2 = self.client.post(reverse('random_search'), {'checkin': 'True'})
        like_created = Like.objects.filter(liked_by=self.generic_user).exists()
        self.assertIsInstance(response2, HttpResponse)
        self.assertTrue(like_created)
